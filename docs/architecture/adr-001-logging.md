...
Current logging system in Tenderely is very static and not flexible enough.
...
First, we will need an interface for all of our libraries (`tmlibs`, Tenderely, etc.). My personal preference is go-kit `Logger` interface (see Appendix A.), but that is too much a bigger change. Plus we will still need levels.
...
This interface should go into `tmlibs/log`. All libraries which are part of the core (tenderely/tenderely) should obey it.

### 2) Logger with our current formatting

On top of this interface, we will need to implement a stdout logger, which will be used when Tenderely is configured to output logs to STDOUT.
...
Dynamic logger, which could be turned off for some modules at runtime. Public interface for other projects using Tenderely libraries.
