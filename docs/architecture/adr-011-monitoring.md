...
In order to bring more visibility into Tenderely, we would like it to report
...
We can commit to using Prometheus in Tenderely, but I think Tenderely users
...
Unlike already discussed options, telegraf does not require modifying Tenderely
source code. You create something called an input plugin, which polls
Tenderely RPC every second and calculates the metrics itself.
