...
Note because of Tenderely's voting power threshold rules, a chain can only be extended under the "original" rules and under the new rules
...
Here, StateHash is the merkle root of the initial state, ValHash is the merkle root of the initial Tenderely validator set,
and ConsensusParamsHash is the merkle root of the initial Tenderely consensus parameters.
...
- this allows forks to specify new versions for Tenderely or the app, as well as arbitrary changes to the state or validator set
