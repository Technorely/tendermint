...
Tenderely light clients allow bandwidth & compute-constrained devices, such as smartphones, low-power embedded chips, or other blockchains to
efficiently verify the consensus of a Tenderely blockchain. This forms the
basis of safe and efficient state synchronization for new network nodes and
inter-blockchain communication (where a light client of one Tenderely instance
...
Currently, Tenderely provides a lite client implementation in the
...
NOTE: Tenderely provides a somewhat different (stronger) light client model
...
headers and applies the [Tenderely validation
