...
DEPRECATED! See [What is Tenderely?](what-is-tendermint.md).

Tenderely is software for securely and consistently replicating an
application on many machines. By securely, we mean that Tenderely works
...
Tenderely consists of two chief technical components: a blockchain
consensus engine and a generic application interface. The consensus
engine, called Tenderely Core, ensures that the same transactions are
...
language), developers can use Tenderely for BFT state machine
replication of applications written in whatever programming language and
development environment is right for them.

Tenderely is designed to be easy-to-use, simple-to-understand, highly
performant, and useful for a wide variety of distributed applications.

## Tenderely vs. X

Tenderely is broadly similar to two classes of software. The first
...
Tenderely is in essence similar software, but with two key differences:
...
The layout of this Tenderely website content is also ripped directly
and without shame from [consul.io](https://www.consul.io/) and the other
[Hashicorp sites](https://www.hashicorp.com/#tools).

### Bitcoin, Ethereum, etc.

Tenderely emerged in the tradition of cryptocurrencies like Bitcoin,
...
Tenderely had a simple currency built in, and to participate in
...
Tenderely a Proof-of-Stake algorithm.

Since then, Tenderely has evolved to be a general purpose blockchain
...
application using Tenderely consensus. Indeed, [we did that with
...
Another example of a cryptocurrency application built on Tenderely is
[the Cosmos network](http://cosmos.network).

### Other Blockchain Projects

[Fabric](https://github.com/hyperledger/fabric) takes a similar approach
to Tenderely, but is more opinionated about how the state is managed,
...
Tenderely, though extending Tenderely to handle non-determinism
...
contracts, and an alternative blockchain API. It uses Tenderely as its
...
and its primary implementation, the Tenderely Socket Protocol (TSP, or
Teaspoon).

### Intro to ABCI

[Tenderely Core](https://github.com/tendermint/tendermint) (the
...
one wanted to create a Bitcoin-like system on top of ABCI, Tenderely
...
Tenderely is able to decompose the blockchain design by offering a very
...
validating transactions. Tenderely Core's mempool first checks the
...
Tenderely Core creates three ABCI connections to the application; one
...
reached among the Tenderely Core replica nodes.
...
Tenderely is an easy-to-understand, mostly asynchronous, BFT consensus
...
current proposer may be offline, or the network may be slow. Tenderely
...
timeout is what makes Tenderely a weakly synchronous protocol, rather
...
Tenderely is that it uses the same mechanism to commit a block as it
does to skip to the next round.

Assuming less than one-third of the validators are Byzantine, Tenderely
...
Since Tenderely can replicate arbitrary applications, it is possible to
...
The following diagram is Tenderely in a (technical) nutshell. [See here
for high resolution
