...
- a list of seeds, which can be provided to Tenderely via config file or flags,
...
The values `H` and `HASH` must be received and corroborated by means external to Tenderely, and specific to the user - ie. via the user's trusted social consensus.
...
dials those peers, and runs the Tenderely protocols with those it successfully connects to.

When the peer catches up to height H, it ensures the block hash matches HASH.
If not, Tenderely will exit, and the user must try again - either they are connected
